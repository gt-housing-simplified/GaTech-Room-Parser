...
def printRoomData(data):
    tableDorms, beds, empty_counts, empty_rooms = [], [], [], []

    for name, data in dorms.items():
        tableDorms.append([name, len(data)])

        if config['empty'] or config['empty_rooms']:
            ...
            empty = {room_type: empty_rooms for room_type, empty_rooms in empty.items() if empty_rooms}
            empty_count = {room_type: len(empty_rooms) for room_type, empty_rooms in empty.items()}

            empty_counts.append(empty_count)
            empty_rooms.append(empty)
    ...
    if config['beds'] or config['empty']:
        for i in range(len(tableDorms)):
            ...
            if config['empty_rooms']:
                print(f"Empty Rooms: {empty_rooms[i]}")
            if config['beds']:
                print(beds[i])
            print()
    else:
